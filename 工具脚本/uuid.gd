...
static var tab:String = "0123456789abcdefABCDEFghijklmnopqrstuvwxyzGHIJKLMNOPQRSTUVWXYZ!@#$%^&*()_+=<>,.?"
static var tab2:String = "0123456789abcdefABCDEF"
...
static func get_uuid()->String:
	...
	for i in randi_range(5,10):
		str += tab[randi_range(0,tab.length() - 1)]
	for i in randi_range(5,8):
		ns += tab2[randi_range(0,tab2.length() - 1)]
	return v5(str,ns)
